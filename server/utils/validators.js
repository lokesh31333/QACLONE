...
const questionValidator = (title, body, tags) => {
  ...
  if (title.trim() === "" || title.length < 15) {
    errors.push("Title must be atleast 15 characters long.");
  }

  if (body.trim() === "" || body.length < 30) {
    errors.push("Question body must be atleast 30 characters long.");
  }

  if (!Array.isArray(tags) || tags.length === 0 || tags.length > 5) {
    errors.push("1-5 tags must be added.");
  }

  if (tags.some((t) => !/^[a-zA-Z0-9-]*$/.test(t))) {
    errors.push("Tags must have alphanumeric characters only.");
  }

  if (tags.filter((t, index) => tags.indexOf(t) !== index).length > 0) {
    errors.push("Duplicate tags cannot be added.");
  }

  if (tags.some((t) => t.length > 15)) {
    errors.push("A single tag can't have more than 15 characters.");
  }
